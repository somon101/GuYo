...
// The set of exercises Admin Web currently knows how to configure. Adding
// "Сопоставление" or a future exercise here later is just one more entry
// in this list -- the backend already stores/serves any exercise_key
// without a schema change (see ExerciseSettings).
const EXERCISES: { key: string; label: string; description: string }[] = [
  {
    key: "true_or_false",
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <ul className="flex flex-col gap-3">
        {EXERCISES.map((ex) => (
          <li key={ex.key}>
            <ExerciseSettingRow exerciseKey={ex.key} label={ex.label} description={ex.description} />
          </li>
        ))}
      </ul>
    </div>
...
function ExerciseSettingRow({
  ...
  useEffect(() => {
    ...
    getExerciseSettings(exerciseKey)
      .then((s) => {
        if (!cancelled) setWordCount(s.word_count);
      })
      .catch(() => {
  ...
  async function handleSave() {
    ...
    try {
      const updated = await setExerciseSettings(exerciseKey, wordCount);
